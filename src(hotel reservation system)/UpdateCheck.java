...
public class UpdateCheck extends JFrame {
	...
	public UpdateCheck() throws SQLException {
		...
		contentPane.add(lblNewLabel);
                ...
                try{
                    conn c = new conn();
                    ResultSet rs = c.statement.executeQuery("select * from ct");
                    while(rs.next()){
		...
		btnUpdate.addActionListener(new ActionListener() {
			public void actionPerformed(ActionEvent ae) { 
				...
				String s2 = txt_ID.getText(); //room_number;    
                                ...
                                c.statement.executeUpdate("update ct set foom = '"+s2+"', name = '"+s3+"', checkintime = '"+s4+"', deposite = '"+s5+"' where number = '"+s1+"'");
		...
		btnAdd.addActionListener(new ActionListener() {
			public void actionPerformed(ActionEvent e) {
				...
				String s1 = c1.getSelectedItem();
				conn c = new conn();
                                ResultSet rs1 = c.statement.executeQuery("select * from ct where number = "+s1);
                                
                                while(rs1.next()){
                                    txt_ID.setText(rs1.getString("foom"));    
                                    txt_Status.setText(rs1.getString("name"));    
                                    txt_Date.setText(rs1.getString("checkintime"));    
                                    txt_Time.setText(rs1.getString("deposite"));    
                                }
                            }catch(Exception ee){}
                            ...
                            try{
                                String total = "";
                                conn c  = new conn();
                                ResultSet rs2 = c.statement.executeQuery("select * from Room where room = "+txt_ID.getText());
                                while(rs2.next()){
